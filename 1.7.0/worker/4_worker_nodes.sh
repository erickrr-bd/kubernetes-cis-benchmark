...
check_4_1_1="4.1.1  - Ensure that the kubelet service file permissions are set to 600 or more restrictive (Automated)"
...
check_4_1_7="4.1.7  - Ensure that the certificate authorities file permissions are set to 600 or more restrictive (Manual)"
...
check_4_1_9="4.1.9  - If the kubelet config.yaml configuration file is being used validate permissions set to 600 or more restrictive (Manual)"
...
if check_argument "$CIS_KUBELET_CMD" '--tls-cipher-suites' >/dev/null 2>&1; then
    ciphers=$(get_argument_value "$CIS_KUBELET_CMD" '--tls-cipher-suites'| cut -d " " -f 1)
